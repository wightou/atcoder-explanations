...
`ツーポインタ法` は `C問題相当`、`データ探索系` の知識記事として扱う。`尺取法` は同じ列上のスライド窓に寄った手法として残し、`ツーポインタ法` とは別記事にする。`尺取法` の slug は `sliding-window` とし、旧 slug `two-pointers` は `create_knowledge_stubs.py` 実行時に `sliding-window` へリネームする。
