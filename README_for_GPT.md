...
- 知識記事本文の `## 関連アルゴリズム` 節では、`###` 見出しが既存知識記事の `title` と完全一致する場合、その見出しを該当記事へのリンクにします。
- Markdownの1行全体が `詳しくは「XXX」の記事参照。` と完全一致し、`XXX` が既存知識記事の `title` と一致する場合、`XXX` だけを該当記事へのリンクにします。
  - 前後の文言が異なる文章、aliases、absorbs は本文自動リンクの対象にしません。
- `category: 典型問題集` は、問題パターンとして参照したい知識記事に使います。
- `DAG上のDP` は `トポロジカルソート` を吸収して扱います。
- `区間DP` と `桁DP` は独立した知識記事として扱い、どちらも `高速化系` に分類します。
