...
尺取法を行う。
`l1` は `diff[l1] < diff[r]-k` である要素を飛ばした直後を表す。
`l2` は `diff[l2] <= diff[r]-k` である要素を飛ばした直後を表す。
したがって、`l2-l1` が `diff[r]-diff[l]=k` となる左端 `l` の個数である。

各 `r` でのポインタの動きは次のようになる。

<!-- table-row-header: true -->
| `r` | `diff[r]` | `diff[r]-k` | `l1` | `l2` | `l2-l1` | この `r` での加算 |
|---:|---:|---:|---:|---:|---:|---:|
| 0 | 0 | -3 | 0 | 0 | 0 | 0 |
| 1 | 1 | -2 | 0 | 0 | 0 | 0 |
| 2 | 3 | 0 | 0 | 1 | 1 | 1 |
